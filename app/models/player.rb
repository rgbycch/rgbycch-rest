...
class Player < ActiveRecord::Base
  ...
  scope :filter_by_name, lambda { |keyword| where("lower(first_name) LIKE ? OR lower(last_name) LIKE ? OR lower(nick_name) LIKE ?", "%#{keyword.downcase}%", "%#{keyword.downcase}%", "%#{keyword.downcase}%" ) }
